...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //                |                  +--- O array "indices[]" contém 3 indices para cada um dos 16 triangulos índices (veja função BuildTriangles()).
...
float* rotate_point(float x, float y, double degree_angle) {
    // v' = Rv
    // |x'|   | cos -sin |   | x |
    // |y'| = | sin  cos | * | y |
    static float new_points[2];

    double rad = degree_angle * 3.14159265358979323846 / 180.0;
    double cos_angle = cos(rad);
    double sin_angle = sin(rad);

    new_points[0] = cos_angle * x - sin_angle * y;
    new_points[1] = sin_angle * x + cos_angle * y;
    return new_points;
}
...
GLuint BuildTriangles()
{
    ...
    for (int i = 1; i <= 16; i++) {
        ...
        //fprintf(stderr, "%d - %f %f - %f %f\n", i, NDC_coefficients[index], NDC_coefficients[index+1],x, y);

        float *new_points = rotate_point(x, y, angle);
        x = *new_points;
        y = *(new_points + 1);
    }
    ...
    for (int i = 1; i < 17; i++) {
        ...
        color_coefficients[index + 3] = 0.0f;  // A
    }
